...
const registerEvents = () => {
	...
	manager.on('sentOfferChanged', (offer, oldState) => {
		if(offer.state == 3) {
			steam.message(offer.partner, "/pre Offer accepted, handling..");
...
exports.createBuyOffer = (partner, price, payment, full, callback) => {
	steam.message(partner,"/me Proccessing your request, please hold.. " )
	...
	community.getSteamUser(new SteamID(partner.toString()), (error, user) => {
		...
		offer.getUserDetails((error, me, them) => {
			...
			Log.Trade("Creating game trade for user : " + partner.toString() + " price :  " + price.toString() + " payments " + payment.toString());

			if(full)
				offer.setMessage("Payment for your unique games. Thank you for using Daskama Shop");
			else
