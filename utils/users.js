...
exports.updatePersona = () => {
	games.getRandomPrice((error, price, rgames) => {
		if(error)
			return Log.Error("Error while updating status: " + error);
...
exports.performBuyCheck = (user, callback) => {
	games.getUnique((error, unique) => {
		...
		offers.getOwnedGames(user, (error, owned) => {
			...
			gems = gems.toFixed(2);
			//add to me 
			let price = (csgo >= 0 ? csgo + " CSGO keys. \n" : "") + (tf >= 0 ? tf + " TF keys.\n" : "") + (gems >= 0 ? gems + " Gems.\n" : "");
		...
		read.on('end', () => {
			...
			games.getUnique((error, unique) => {
				...
				offers.getOwnedGames(user, (error, owned) => {
					...
					if(diff.length <= 0)
						return callback("I don't have any games that you don't already own. Sorry :(");
...
exports.handle = (user, callback) => {
	...
	read.on('end', () => {
		...
		if(!json[user])
			return callback("You have not bought game yet, use !buy or !buytf");
		...
		steam.message(user, "Thanks for accepting the trade, here are your keys:\n " + keymsgs.join("\n") + " \n If you have a problem, please contact support .  \n discord : discord.gg/ZjxtdHM  \n " + "HelpDesk: https://www.hulibrci.wtf/ticket/public/en/  \n" + "\n" + "Thanks for use my services, it would be realy cool if you leave me a +rep comment :D \n");
        //msg admin
	    let admin = "76561198098119891"; //here you steam ID - is a notification   76561198098119891 == Daskama
		let msg = " We just sold  " + keys.length.toString() + " games " + " to " + "https://steamcommunity.com/profiles/" + user.toString();	
		steam.message(admin,msg);
        //log msg 
		let tardelog = "We sold to the user " + user.toString() + " with " + keys.length.toString() + " games " ;
		Log.Trade(tardelog);
